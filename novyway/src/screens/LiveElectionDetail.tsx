...
import { useResourcePermissions } from '../tenancy/access'
...
export function LiveElectionDetail({ electionId }: { electionId: string }) {
  ...
  const resourceAccess = useResourcePermissions('election', electionId)
  const canSeeSubject = resourceAccess.permissions?.subject === true
  const canParticipate = resourceAccess.permissions?.participate === true
  const canSeeResults = resourceAccess.permissions?.results === true
  const canSeeBallots = resourceAccess.permissions?.ballots === true
  const election = data?.elections.find((item) => item.id === electionId)
  const registry = useDocumentProposals({ electionId, enabled: canSeeSubject })
  ...
  useEffect(() => {
    let active = true
    if (!canSeeSubject || !proposal || !election) {
    ...
    return () => { active = false }
  }, [canSeeSubject, document, election, proposal])
  const category = data?.categories.find((item) => item.id === election?.categoryId)

  if (resourceAccess.loading) return <div className="empty">{ru ? 'Проверяем доступ…' : 'Checking access…'}</div>
  if (resourceAccess.permissions?.discover === false) return <div className="callout red">{ru ? 'У вас нет доступа к этому голосованию.' : 'You do not have access to this election.'}</div>
  ...
  const verifiedProposal = canSeeSubject && integrity === 'verified' ? proposal : undefined
  ...
  const subjectVerificationPending = canSeeSubject && (registry.loading || Boolean(proposal && integrity === 'checking'))
  const proposalAllowsVoting = canSeeSubject && !subjectVerificationPending && !registry.error && (!proposal || integrity === 'verified')
  ...
  return <>
    ...
    <div className="grid c2">
      <div className="stack">
        {!canSeeSubject && <div className="callout cyan">{ru ? 'Предмет решения доступен только выбранным участникам.' : 'The decision subject is available only to selected participants.'}</div>}
        {canSeeSubject && registry.loading && <div className="empty">{ru ? 'Проверяем предмет решения…' : 'Verifying the subject…'}</div>}
        {canSeeSubject && registry.error && <div className="callout red">{ru ? 'Не удалось проверить предмет решения. Бюллетень закрыт до восстановления связи с реестром.' : 'The subject could not be verified. The ballot is closed until the registry is available.'} <button className="btn small" onClick={registry.refresh}>{ru ? 'Повторить' : 'Retry'}</button></div>}
        {canSeeSubject && !registry.loading && proposal && integrity === 'checking' && <div className="callout cyan">{ru ? 'Сверяем хеш и параметры поправки с Aptos…' : 'Checking the amendment hash and parameters against Aptos…'}</div>}
        {canSeeSubject && proposal && integrity === 'mismatch' && <div className="callout red">{ru ? 'Текст поправки не совпал с хешем или параметрами голосования Aptos. Голосование заблокировано для безопасной проверки.' : 'The amendment text does not match its hash or Aptos election parameters. Voting is blocked for safety.'}</div>}
        {verifiedProposal && (
          <Panel title={ru ? 'Предмет решения' : 'Subject of the decision'} hint={(ru ? 'голосование №' : 'election #') + election.id}>
        ...
        <Panel title={ru ? 'Параметры голосования' : 'Election parameters'}>
          <div className="stack">
            <KV k={ru ? 'Создатель' : 'Creator'} v={shortAddr(election.createdBy)} mono />
            {canSeeSubject && <KV k={ru ? 'Метаданные' : 'Metadata'} v={verifiedProposal ? <a href={verifiedProposal.metadataUri} target="_blank" rel="noreferrer">{shortAddr(election.metadataHash)} ↗</a> : shortAddr(election.metadataHash)} mono />}
      ...
      <div className="stack">
        {canSeeResults && <Panel title={ru ? 'Итог' : 'Result'}>
        ...
        {canSeeBallots && <Panel title={ru ? 'Распределение голосов' : 'Ballot distribution'}>
          ...
          <div style={{ marginTop: 14 }}><KV k={ru ? 'Уникальные участники' : 'Unique voters'} v={election.uniqueVoters} mono /></div>
        </Panel>}
        <Panel title={ru ? 'Независимая проверка' : 'Independent verification'}>
          <p className="muted">{ru ? 'Откройте доступные записи в обозревателе Aptos и сравните адрес модуля, события и итоговые числа.' : 'Open the available Aptos Explorer records and compare the module address, events, and totals.'}</p>
          <div className="row" style={{ marginTop: 12 }}>
            {canSeeSubject && tx?.created && <a className="btn small" href={aptosTestnetExplorer(`txn/${tx.created}`)} target="_blank" rel="noreferrer">{ru ? 'Создание' : 'Creation'} ↗</a>}
            {canSeeResults && tx?.finalized && <a className="btn small" href={aptosTestnetExplorer(`txn/${tx.finalized}`)} target="_blank" rel="noreferrer">{ru ? 'Завершение' : 'Finalization'} ↗</a>}
        ...
        {!canParticipate
          ? election.status === 'active' && <div className="callout cyan">{ru ? 'Голосование можно наблюдать, но участие доступно только выбранным участникам.' : 'You can observe this election, but participation is limited to selected participants.'}</div>
          : !canSeeSubject
            ? <div className="callout red">{ru ? 'Бюллетень закрыт: политика участия шире видимости предмета. Администратору нужно исправить правила.' : 'The ballot is closed because participation is broader than subject visibility. An administrator must fix the policy.'}</div>
